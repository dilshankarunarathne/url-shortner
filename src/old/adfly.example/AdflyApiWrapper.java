...
public class AdflyApiWrapper {
	...
	private AdflySimpleRestClient rest;
	
	public AdflyApiWrapper() {
		rest = new AdflySimpleRestClient(BASE_SCHEME, BASE_HOST);
	}
	
	public String getGroups(int page) {
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("_page", page);
		prepareParams(AuthType.HMAC, params);
	 ...
	 * @param params
	 */
	private void prepareParams(AuthType authType, Map<String, Object> params) {
		params.put("_user_id", USER_ID);
		params.put("_api_key", PUBLIC_KEY);
		
		if (authType.equals(AuthType.BASIC)) {
			
		} else if (authType.equals(AuthType.HMAC)) {
	...
	private void prepareParams(Map<String, Object> params) {
		prepareParams(AuthType.BASIC, params);
	}
